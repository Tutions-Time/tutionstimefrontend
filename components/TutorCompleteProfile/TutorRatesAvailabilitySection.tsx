...
export default function TutorRatesAvailabilitySection({ errors }: { errors: Record<string, string> }) {
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-8">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <Input
            ...
            onChange={e => dispatch(setField({ key: "hourlyRate", value: e.target.value }))}
            placeholder="e.g., 500"
            className="h-10"
          />
          {errors.hourlyRate && <p className="text-rose-600 text-xs mt-1">{errors.hourlyRate}</p>}
        </div>
        <div>
          <Label>Monthly Rate (₹)</Label>
          <Input
            value={profile.monthlyRate}
            onChange={e => dispatch(setField({ key: "monthlyRate", value: e.target.value }))}
      ...
      <Label className="flex items-center gap-2 mb-2">
        <CalendarIcon className="w-4 h-4 text-primary" /> Availability (Select Dates)
      </Label>
      <AvailabilityPicker
