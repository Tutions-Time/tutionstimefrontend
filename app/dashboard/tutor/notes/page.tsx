...
export default function TutorNotesPage() {
  ...
  const submit = async () => {
    try {
      ...
      if (res?.success) { toast({ title: 'Note created' }); setTitle(''); setDescription(''); setSubject(''); setClassLevel(''); setBoard(''); setPrice(''); setKeywords(''); setPdfFile(null); setOpen(false); load(); }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="lg:pl-64">
        <Topbar
          ...
          action={<Button onClick={()=>setOpen(true)}>Add Note</Button>}
        />
            ...
            {items.map((n:any)=> (
              <Card key={String(n._id)} className="p-4 bg-white rounded-2xl">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-sm text-muted">₹{Number(n.price||0)}</div>
                </div>
                <div className="text-xs text-muted mb-2">{n.subject} • {n.classLevel} • {n.board}</div>
                <div className="mb-3">
                  <div className="border rounded-lg overflow-hidden">
                    <iframe src={n.pdfUrl} className="w-full h-40" />
                  </div>
                </div>
                <div className="mt-2 flex gap-2">
                  <Button size="sm" variant="outline" onClick={()=>{ setPreviewNote(n); setPreviewOpen(true); }}>Preview</Button>
                  <Button size="sm" variant="destructive" onClick={()=>remove(String(n._id))}>Delete</Button>
                </div>
              </Card>
            ))}
            {items.length===0 && <Card className="p-6 text-center text-muted">No notes yet.</Card>}
          </div>
        </main>
      </div>
        ...
        <DialogContent className="max-w-4xl">
          ...
          {previewNote && (
            <div className="h-[70vh]">
              <iframe src={previewNote.pdfUrl} className="w-full h-full" />
            </div>
          )}
        </DialogContent>
      </Dialog>
