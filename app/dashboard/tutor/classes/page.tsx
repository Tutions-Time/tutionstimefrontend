...
function RegularRescheduleInline({ sessionId, onAfter }: { sessionId: string; onAfter?: () => Promise<void> | void }) {
  ...
  const [meridiem, setMeridiem] = React.useState<"AM" | "PM">("PM");
      ...
      {!open ? (
        ...
        <div className="space-y-2 border rounded-lg p-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleSubmit} disabled={!date || submitting} className="px-3 py-2 rounded-lg text-sm bg-[#FFD54F] text-black">{submitting ? "Submitting..." : "Submit"}</button>
