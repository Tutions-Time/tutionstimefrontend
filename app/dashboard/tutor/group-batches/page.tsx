...
export default function TutorGroupBatchesPage() {
  ...
  const [form, setForm] = useState<any>({ subject: "", level: "", batchType: "revision", scheduleType: "fixed", fixedDates: [], seatCap: 10, pricePerStudent: 500, description: "", published: true });
  ...
  const create = async () => {
    try {
      ...
      if (res.data?.success) {
        ...
        setForm({ subject: "", level: "", batchType: "revision", scheduleType: "fixed", fixedDates: [], seatCap: 10, pricePerStudent: 500, description: "", published: true });
        load();
      } else {
        toast.error("Failed");
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="lg:pl-64">
        <Topbar title="Group Batches" subtitle="Create and manage your batches" action={
        ...
        <main className="p-4 lg:p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {list.map((b:any)=> (
              <div key={b._id} className="border rounded p-3 bg-white shadow-sm">
                <div className="font-medium">{b.subject} • {b.level}</div>
                <div className="text-sm">Seats: {b.liveSeats}/{b.seatCap}</div>
                <div className="flex gap-2 mt-2">
                  <a className="bg-blue-600 text-white px-3 py-2 rounded" href={`/dashboard/tutor/group-batches/${b._id}`}>View</a>
